...
function webform($user_auth, $application, $documento, $name_value_list, $module_name) {
...
//        function webform($documento, $name_value_list, $module_name)
...
//                            $name_value_list['nrodocumento_c']=$documento;
        ...
        foreach($name_value_list as $value){
            if($value['name'] == 'email' && isset($value['value']) && strlen($value['value']) > 0){
                $verificarMail = '';
                $sqlVerificarMail=$db->query('SELECT ea.id email_id 
                                              FROM email_addresses ea,email_addr_bean_rel br 
                                              WHERE ea.id=br.email_address_id AND bean_module="Accounts" AND email_address="'.(utf8_decode($value['value'])).'"
                                                    AND ea.deleted=0 AND br.deleted=0'); 
                while($a = $db->fetchByAssoc($sqlVerificarMail))
                   $verificarMail=$a['email_id'];
                
                if($verificarMail != ''){
                    $jsonEstudiante = json_encode($name_value_list);
                    $r=$db->query('INSERT INTO historico 
                                   SET (id, parent_id, bean_module, fecha, origen, datos) 
                                   VALUES (O, 0, "Accounts",'.date("Y-m-d H:i:s").',"Web Service","'.$jsonEstudiante.'")');
                    
                    $error->set_error('no_access');
                    return "Estdiante duplicado";
                }
            ...
            if($value['name'] == 'id' && isset($value['value']) && strlen($value['value']) > 0){
                ...
                if(is_null($result)){
                    ...
                    break;
                        ...
                        return "Se inserto el estudiante";
...
function outlooklatino($user_auth, $application, $documento, $name_value_list, $module_name,$email) {
    // perform your logic here and return a string
    // Comienzo de la logica de login
    global $sugar_config, $system_config;
