...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if (isset($GLOBALS['check_notify'])) {
        ...
        $new=true;
        if (isset($focus->opportunity_type)){
            $new=false;
        }
        ...
        if (file_exists('custom/include/clases/varios/ManejoEtapasVenta.php')){
            ...
            $manejo=new ManejoEtapasVenta($focus->sales_stage);
            
            
                $sql = "SELECT sales_stage FROM opportunities WHERE id='".$focus->id."'";
                $result=$db->query($sql);
                $estadoAnterior='';
                while ($row = $db->fetchByAssoc($result)) 
                    $estadoAnterior=$row['sales_stage'];
                
                $estadoActual=$manejo->prepareSaveStage($focus->sales_stage);
                ...
                if($estadoAnterior == $estadoActual){
                    ...
                    foreach ($datos as $key => $value) {
                        ...
                        $estadoPorArchivo = $manejo->prepareSaveStage($datos['tipo_archivo_' . $id]);
                    }
                    if(($estadoAnterior == 'application_sent') && 
                      (($estadoPorArchivo == 'application_unsuccessful')||($estadoPorArchivo == 'conditional_offer')
                      ||($estadoPorArchivo == 'unconditional_offer')||($estadoPorArchivo == 'coe_received'))){
                        $focus->sales_stage = $estadoPorArchivo;
                        $sql1 = "UPDATE etapa_venta 
                                 SET deleted = 1
                                 WHERE id_oportunidad='".$focus->id."'";
                    ...
                    if(($estadoAnterior == 'conditional_offer') && 
                      (($estadoPorArchivo == 'unconditional_offer')||($estadoPorArchivo == 'Closed Lost')
                      ||($estadoPorArchivo == 'deferral_process'))){
                        $focus->sales_stage = $estadoPorArchivo;
                        $sql1 = "UPDATE etapa_venta 
                                 SET deleted = 1
                                 WHERE id_oportunidad='".$focus->id."'";
                    ...
                    if(($estadoAnterior == 'unconditional_offer') && 
                      (($estadoPorArchivo == 'Closed Lost')||($estadoPorArchivo == 'acceptance_sent')
                      ||($estadoPorArchivo == 'coe_received'))){
                        $focus->sales_stage = $estadoPorArchivo;
                        $sql1 = "UPDATE etapa_venta 
                                 SET deleted = 1
                                 WHERE id_oportunidad='".$focus->id."'";
                    ...
                    if(($estadoAnterior == 'acceptance_sent') && ($estadoPorArchivo == 'coe_received')){
                        $focus->sales_stage = $estadoPorArchivo;
                        $sql1 = "UPDATE etapa_venta 
                                 SET deleted = 1
                                 WHERE id_oportunidad='".$focus->id."'";
                    ...
                    if(($estadoAnterior == 'coe_received') && 
                      (($estadoPorArchivo == 'Closed Lost')||($estadoPorArchivo == 'visa_sent'))){
                        $focus->sales_stage = $estadoPorArchivo;
                        $sql1 = "UPDATE etapa_venta 
                                 SET deleted = 1
                                 WHERE id_oportunidad='".$focus->id."'";
                    ...
                    if(($estadoAnterior == 'visa_sent') && 
                      (($estadoPorArchivo == 'visa_denied')||($estadoPorArchivo == 'visa_received'))){
                        $focus->sales_stage = $estadoPorArchivo;
                        $sql1 = "UPDATE etapa_venta 
                                 SET deleted = 1
                                 WHERE id_oportunidad='".$focus->id."'";
                    ...
                    if(($estadoAnterior == 'visa_received') && 
                      (($estadoPorArchivo == 'deferral_process')||($estadoPorArchivo == 'Closed Lost')
                      ||($estadoPorArchivo == 'Closed Won'))){
                        $focus->sales_stage = $estadoPorArchivo;
                        $sql1 = "UPDATE etapa_venta 
                                 SET deleted = 1
                                 WHERE id_oportunidad='".$focus->id."'";
                    ...
                    if(($estadoAnterior == 'application_incomplete') && 
                      (($estadoPorArchivo == 'application_sent')||($estadoPorArchivo == 'Closed Lost'))){
                        $focus->sales_stage = $estadoPorArchivo;
                        $sql1 = "UPDATE etapa_venta 
                                 SET deleted = 1
                                 WHERE id_oportunidad='".$focus->id."'";
                    ...
                    if(($estadoAnterior == 'deferral_process') && ($estadoPorArchivo == 'unconditional_offer')){
                        $focus->sales_stage = $estadoPorArchivo;
                        $sql1 = "UPDATE etapa_venta 
                                 SET deleted = 1
                                 WHERE id_oportunidad='".$focus->id."'";
...
//            if ($new){
//                $oportunidadWorkflow=new OportunidadWorkFlow('oportunidad','');
//                $oportunidadWorkflow->procesaWorkFlow($focus->id, $focus->module_name, '', $focus->sales_stage,"manual" );
//            }else{
//                $oportunidadWorkflow=new OportunidadWorkFlow('oportunidad','unconditional_offer');
//                $oportunidadWorkflow->procesaWorkFlow($focus->id, $focus->module_name, 'unconditional-acceptance', 'unconditional_offer',"manual",false );
//            }
                
        }
        
        //Upload Files
        $path= 'include/SugarFiles/SugarFiles.php';
           if (file_exists($path)){
              include_once($path);
              $file=new SugarFiles();
              $file->save($return_id,$focus->module_dir);
           }
